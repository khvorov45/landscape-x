...
void
GroupAlign(int nseq1, int nseq2, char** name, int* nlen, char** seq, char** aseq, char** mseq1, char** mseq2, int*** topol, double** len, double* eff, int alloclen) {
    int           i;
    int           clus1, clus2;
    int           s1, s2;
    double        pscore;
...
WriteOptions(FILE* fp) {
    ...
    if (scoremtx == 0)
        ...
        fprintf(fp, "Dayhoff( machigai ga aru )\n");
    else if (scoremtx == 2)
        fprintf(fp, "M-Y\n");
    else if (scoremtx == -1)
        fprintf(fp, "DNA\n");
...
main(int argc, char* argv[]) {
    ...
    gp1 = fopen(argv2[0], "r");
    if (!gp1)
        ErrorExit("cannot open file1");
    gp2 = fopen(argv2[1], "r");
    if (!gp2)
        ErrorExit("cannot open file2");
...
#else
    njob = nseq2;
    readDataforgaln(gp2, name + nseq1, nlen + nseq1, seq2);
    njob = nseq1;
    readDataforgaln(gp1, name, nlen, seq1);
#endif
    njob = nseq1 + nseq2;
