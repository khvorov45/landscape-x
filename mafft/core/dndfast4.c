...
howmanyx(char* s) {
    ...
    if (scoremtx == -1) {
        do {
            if (!strchr("atgcuATGCU", *s))
                val++;
        } while (*++s);
    } else {
        do {
            if (!strchr("ARNDCQEGHILKMFPSTWYV", *s))
                val++;
        } while (*++s);
    }
    return (val);
...
main(int argc, char* argv[]) {
    ...
    WriteForFasta(hat2p, njob, (char**)name1, nlen1, seq);
    fclose(hat2p);
    ...
    for (i = 0; i < njob; i++) {
        ...
        if (!hat2p)
            ErrorExit("Cannot open datafile.");
        WriteForFasta(hat2p, njob - i, (char**)(name1 + i), nlen1 + i, seq + i);
        fclose(hat2p);
        ...
        WriteForFasta(hat2p, 1, (char**)(name1 + i), nlen1, seq1);
        fclose(hat2p);

        if (scoremtx == -1)
            sprintf(com, "fasta3 -n -Q -h -b%d -E%d -d%d %s %s %d > %s", M, M, 0, queryfile, datafile, ktuple, fastafile);
        else
            sprintf(com, "fasta3 -Q -h -b%d -E%d -d%d %s %s %d > %s", M, M, 0, queryfile, datafile, ktuple, fastafile);
        res = system(com);
        if (res)
            ErrorExit("error in fasta");
