...
static int
varpairscore(int nseq, int npick, int nlenmax, char** seq, int seed) {
    int    i, j, npair;
    int*   slist;
    char** pickseq;
    double score;
    ...
    for (i = 0; i < npick; i++)
        gappick0(pickseq[i], seq[slist[i]]);
...
static void
pickup(int n, int* seqlen, int*** topol, char** name, char** seq)  // memsave ni mitaiou
{
    int    i, j, k, m;
    int**  longestseq;
    int**  longestlen;
...
void
makecompositiontable_p(int* table, int* pointt) {
    int point;
...
void
makepointtable_nuc_dectet(int* pointt, int* n) {
    int           point;
    register int* p;
...
void
makepointtable_nuc_octet(int* pointt, int* n) {
    int           point;
    register int* p;
...
void
makepointtable_nuc(int* pointt, int* n) {
    int           point;
    register int* p;
...
void
makepointtable(int* pointt, int* n) {
    int           point;
    register int* p;
...
static double
preferenceval(int ori, int pos, int max)  // for debug
{
    pos -= ori;
    if (pos < 0)
        pos += max;
...
static void*
compactdisthalfmtxthread(void* arg)  // enablemultithread == 0 demo tsukau
{
    ...
    while (1) {
        ...
        if (!table1)
            ErrorExit("Cannot allocate table1\n");
        if (i % 100 == 0) {
            if (nthreadpair)
...
static void*
ylcompactdisthalfmtxthread(void* arg)  // enablemultithread == 0 demo tsukau
{
    compactdistmtxthread_arg_t* targ = (compactdistmtxthread_arg_t*)arg;
    int                         njob = targ->njob;
    int                         thread_no = targ->thread_no;
...
static void*
msacompactdisthalfmtxthread(void* arg)  // enablemultithread == 0 demo tsukau
{
...
static void*
ylmsacompactdisthalfmtxthread(void* arg)  // enablemultithread == 0 demo tsukau
{
    msacompactdistmtxthread_arg_t* targ = (msacompactdistmtxthread_arg_t*)arg;
    int                            njob = targ->njob;
    int                            thread_no = targ->thread_no;
...
#if 1
static void*
msadistmtxthread(void* arg)  // enablemultithread == 0 demo tsukau
{
...
static void
recountpositions(ExtAnch* pairanch, int n1, int n2, char** seq1, char** seq2)  // loop no junban kentou
{
    int  i, j, k, len, pos;
    int* map;
...
static int
anchidcomp(const void* p, const void* q) {
...
static int
anchcomp(const void* p, const void* q) {
...
static int
anchscorecomp(const void* p, const void* q) {
...
static void
indexanchors(ExtAnch* a, int** idx) {
    int n;
    for (n = 0; a[n].i > -1; n++)
        ;
...
static void
checkanchors_strongestfirst(ExtAnch* a, int s, double gapratio1, double gapratio2) {
    int    p, q;
    double zureij;
...
static void
pickpairanch(ExtAnch** pairanch, ExtAnch* extanch, int** anchindex, int n1, int n2, int* m1, int* m2, char** seq1, char** seq2)  // loop no junban wo kaeta hou ga iikamo
{
    int i, j, k, s;
    s = 0;
#if 0
            ...
            switch (alg) {
                ...
                case ('M'):
                    if (l < 500 || l % 100 == 0)
                        reporterr("m");
                    if (l < 500 || l % 100 == 0)
                        if ((cpmxchild1 && *cpmxchild1) || (cpmxchild0 && *cpmxchild0))
                            reporterr(" h");
...
#if SKIP
            FreeIntMtx(skiptable1);
            skiptable1 = NULL;
            FreeIntMtx(skiptable2);
            skiptable2 = NULL;
#endif
        }
...
static int
dooneiteration(int* nlen, char** aseq, int nadd, char* mergeoralign, char** mseq1, char** mseq2, int*** topol, Treedep* dep, int** memhist, double*** cpmxhist, double* effarr, double** newdistmtx, int* selfscore, ExtAnch* extanch, int** anchindex, int* alloclen, int (*callback)(int, int, char*)) {
    ...
    if (callback && callback(0, 50, "Progressive alignment"))
        goto chudan_tbfast;
...
//		clus2 = fastconjuction_noweight( topol[l][1], aseq, mseq2, effarr2,  indication2 );
        ...
        for (i = 0; i < clus1; i++)
            strcpy(oseq1[i], mseq1[i]);
        for (i = 0; i < clus2; i++)
            strcpy(oseq2[i], mseq2[i]);
        ...
        if (l < 500 || l % 100 == 0)
            reporterr("\rIteration % 5d / %d ", ll + 1, njob * ITERATIVECYCLE);
        if (callback && callback(0, 50 + 50 * l / (njob - 1), "Progressive alignment"))
            goto chudan_tbfast;
#if 0
		reporterr( "\nclus1=%d, clus2=%d\n", clus1, clus2 );
...
*/
        ...
        if (nscore < oscore) {
            for (i = 0; i < clus1; i++)
                strcpy(mseq1[i], oseq1[i]);
            for (i = 0; i < clus2; i++)
                strcpy(mseq2[i], oseq2[i]);
        }
        FreeCharMtx(oseq1);
        FreeCharMtx(oseq2);
...
static int
treebase(int* nlen, char** aseq, int nadd, char* mergeoralign, char** mseq1, char** mseq2, int*** topol, Treedep* dep, int** memhist, double*** cpmxhist, double* effarr, double** newdistmtx, int* selfscore, ExtAnch* extanch, int** anchindex, int* alloclen, int (*callback)(int, int, char*)) {
...
preparepartmtx(int nseq) {
    ...
    if (compacttree == 1) {
        ...
        if (i == nseq)
            reporterr("The full matrix will be used.\n");
...
int
disttbfast(int ngui, int lgui, char** namegui, char** seqgui, int argc, char** argv, int (*callback)(int, int, char*)) {
    int*      nlen = NULL;
    int*      nogaplen = NULL;
        ...
        if (compacttree) {
...
#endif
            {
                for (i = 0; i < njob; i++) {
                    ...
                    if (i % 100 == 0) {
                        reporterr("\r% 5d / %d", i + 1, njob);
                        if (callback && callback(0, i * 25 / njob, "Distance matrix"))
                            goto chudan;
                    }
                    makecompositiontable_p(table1, pointt[i]);
...
//							reporterr(       "HIT!\n" );
        ...
        if (calcpairdists)
            for (i = 0; i < njob; i++)
                selfscore[i] = naivepairscore11(seq[i], seq[i], penalty_dist);  // (int)?
...
#endif
        {
...
#endif
            {
                ...
                if (treebase(nlen, bseq, nadd, mergeoralign, mseq1, mseq2, topol, dep, memhist, cpmxhist, eff, NULL, NULL, extanch, anchindex, &alloclen, callback))
                    goto chudan;
            }
        }
        ...
        //		use_getrusage();
        reporterr("\ndone.\n\n");
        if (callback && callback(0, 100, "Progressive alignment"))
            goto chudan;
        ...
        if (cpmxhist[njob - 2]) {
            ...
            FreeDoubleMtx(cpmxhist[njob - 2]);
            cpmxhist[njob - 2] = NULL;
        }
        free(cpmxhist);
        cpmxhist = NULL;
#endif
...
main(int argc, char** argv) {
    ...
    if (res == GUI_CANCEL)
        res = 0;  // treeout de goto chudan wo riyousuru
    return res;
}
