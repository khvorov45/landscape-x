...
char**
align0(double* wm, char** aseq, char* seq, double effarr[M], int icyc, int ex) {
    char** result;
...
double
score_m_1_0(char** aseq, int locnjob, int s, double** eff, double effarr[M]) {
    double x;
...
int
iteration(int locnjob, char name[M][B], int nlen[M], char** aseq, char** bseq, int*** topol, double** len, double** eff) {
    ...
    if (!tbitr && !tbweight) {
        writePre(locnjob, (char**)name, nlen, aseq, 0);
...
*/
        for (i = locnjob - 2; i >= s; i--)
            strcpy(mseq2[i + 1], mseq2[i]);
