...
static void
cnctintvec(int* res, int* o1, int* o2) {
    while (*o1 != -1)
        *res++ = *o1++;
    while (*o2 != -1)
        *res++ = *o2++;
...
static void
countnewres(int len, Blocktorealign* realign, int* posmap, int* gaplist) {
...
static int
lencomp(const void* a, const void* b)  // osoikamo
{
    char** ast = (char**)a;
    char** bst = (char**)b;
    int    lena = strlen(*ast);
...
static int
dorealignment_tree(Blocktorealign* block, char** fullseq, int* fullseqlenpt, int norg, int*** topol, int* follows) {
...
static void
adjustposmap(int len, int* posmap, int* gaplist) {
    int* newposmap;
    int *mpt1, *mpt2;
    int  lenbk, zure;
...
static int
insertgapsbyotherfragments_compact(int len, char* a, char* s, int* l, int* p) {
    int gaplen;
    int i, pos, pi;
    int prevp = -1;
    int realignment = 0;
...
void
makegaplistcompact(int len, int* p, int* c, int* l) {
    int i;
    int pg;
    int prep = -1;
    for (i = 0; i < len + 2; i++) {
...
void
gaplist2alnx(int len, char* a, char* s, int* l, int* p, int lenlimit) {
    int   gaplen;
    int   pos, pi, posl;
    int   prevp = -1;
    int   reslen = 0;
...
static double
treebase(int nseq, int* nlen, char** aseq, int nadd, char* mergeoralign, char** mseq1, char** mseq2, int*** topol, double* effarr, int* alloclen, LocalHom** localhomtable, RNApair*** singlerna, double* effarr_kozo) {
    int         i, l, m;
    ...
    if (constraint) {
        ...
        if (multidist) {
            for (i = 0; i < nseq; i++)
                localhomshrink[i] = (LocalHom**)calloc(1, sizeof(LocalHom*));
        } else
#endif
        {
            for (i = 0; i < nseq; i++)
                localhomshrink[i] = (LocalHom**)calloc(nseq, sizeof(LocalHom*));
        }
    }
...
//		fflush( stderr );
        ...
        if (strlen(indication1) > 66)
            fprintf(stderr, "...");
        fprintf(stderr, "\n");
        fprintf(stderr, "group2 = %.66s", indication2);
        if (strlen(indication2) > 66)
            fprintf(stderr, "...");
        fprintf(stderr, "\n");
#endif
...
static void
mtxcpy(int norg, int njobc, double*** iscorec, double** iscore) {
    int     i, nlim, n;
    double *fpt, *fptc;
...
addsinglethread(void* arg) {
    ...
    {
        for (i = 0; i < njobc; i++)
            nogaplenjusttodecideaddhereornot[i] = 0;
    } else {
        for (i = 0; i < norg; i++) {
    ...
    if (constraint) {
        ...
        if (multidist) {
            for (i = 0; i < njobc; i++)
                localhomtablec[i] = (LocalHom*)calloc(1, sizeof(LocalHom));  // motto chiisaku dekiru.
        } else
#endif
        {
            for (i = 0; i < njobc; i++)
                localhomtablec[i] = (LocalHom*)calloc(njobc, sizeof(LocalHom));  // motto chiisaku dekiru.
            for (i = 0; i < norg; i++)
                for (j = 0; j < norg; j++)
                    localhomtablec[i][j] = localhomtable[i][j];  // iru!
        }
    }
    ...
    while (1) {
        ...
        if (multidist || tuplesize > 0) {
            for (i = 0; i < norg; i++)
                iscorec[i][norg - i] = nscore[i][iadd];
        } else {
            for (i = 0; i < norg; i++)
                iscorec[i][norg - i] = iscore[i][norg + iadd - i];
        }
...
//			fprintf( stdout, "\n" );
    ...
    if (mergeoralign)
        free(mergeoralign);
    mergeoralign = NULL;
    if (nogaplenjusttodecideaddhereornot)
        free(nogaplenjusttodecideaddhereornot);
    nogaplenjusttodecideaddhereornot = NULL;
    if (alnleninnode)
        free(alnleninnode);
    alnleninnode = NULL;
    if (tmpseq)
...
void
ktupledistancematrix(int nseq, int norg, int nlenmax, char** seq, char** name, double** imtx, double** nmtx) {
    char*   tmpseq;
    int*    grpseq;
    int**   pointt;
    int     i, j;
...
void
dndpre(int nseq, char** seq, double** mtx)  // not used yet
{
    int     i, j, ilim;
    double* selfscore;
    double  mtxv;
...
static void
smoothing2(int njob, int nadd, int lenfull, char** seq, Blocktorealign* realign) {
...
static void
smoothing1(int njob, int nadd, int lenfull, char** seq, Blocktorealign* realign) {
    int i, j, norg = njob - nadd;
...
static double
ratioambiguouscolumn_simple(char** s, int n, int l) {
    int i, j;
    int nunambiguous;
    int nambiguous;
    if (dorp == 'd')
...
//		outlocalhom( localhomtable, njob );
    ...
    if (multidist) {
        ...
        if (constraint)
            FreeLocalHomTable_one(localhomtable, norg, nadd);
    } else
#endif
    {
        if (constraint)
