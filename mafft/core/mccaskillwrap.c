...
void
outmccaskill(FILE* fp, RNApair** pairprob, int length) {
    int      i;
    RNApair* pt;
    for (i = 0; i < length; i++)
        for (pt = pairprob[i]; pt->bestpos != -1; pt++) {
            if (pt->bestpos > i)
...
#if 1
static void
readrawmccaskill(FILE* fp, RNApair** pairprob, int length) {
    char   gett[1000];
    int*   pairnum;
    int    i;
    int    left, right;
    ...
    while (1) {
        ...
        if (prob < 0.01)
            continue;  // mxscarna to mafft ryoho ni eikyou
...
main(int argc, char* argv[]) {
    ...
    for (i = 0; i < njob; i++) {
        for (j = 0; j < nlen[i]; j++)
            for (pairprobpt = pairprob[i][j]; pairprobpt->bestpos != -1; pairprobpt++) {
                left = gapmap[i][j];
