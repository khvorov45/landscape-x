...
selfdpthread(void* arg) {
    ...
    while (1) {
        ...
        if (nthread) {
            ...
            j = *jshare;
            if (j % 100 == 0)
                reporterr("%d / %d (thread %d)   \r", j, iend, thread_no);
            if (j == iend) {
...
static int
localcommonsextet_p2(short* table, int* pointt) {
    int    value = 0;
    short  tmp;
    int    point;
    short* memo;
...
static void
makecontrastorder6mer(int* order, int** pointt, int** pointt_rev, char** seq, int iend, int shift) {
    int          i;
    double*      res;
    contrastarr* arr;
...
static void
makecontrastorder(int* order, char** seq, int iend, int shift) {
    int          i;
    double*      res;
    contrastarr* arr;
    ...
    if (alg == 'G')  // dp to the first sequence
    {
        ...
        for (i = istart; i < njob; i++) {
            ...
            if (!dodp) {
                ...
                table1_rev = (short*)calloc(tsize, sizeof(short));
                if (!table1_rev)
                    ErrorExit("Cannot allocate table1_rev\n");
                makecompositiontable_p(table1, pointt[ic]);
                makecompositiontable_p(table1_rev, pointt_rev[ic]);
            }
#endif
...
//				reporterr( "iend = %d -> %d\n", iend, reflim );
            ...
            if (nthread) {
                ...
                *jsharept = 0;
                ...
                if (i % 100 == 1)
                    fprintf(stderr, " %d / %d (%d threads)   \r", i, njob, nthread);
                for (j = 0; j < nthread; j++) {
                    targ[j].iend = iend;
                    targ[j].map = map;
                    targ[j].seq = seq;
            ...
            //			reporterr( "i=%d, res_reverse = %f\n", i, res_reverse );
                ...
                res_reverse = 0.0;
                for (j = 0; j < iend; j++)
                    if (res_reverse < (double)resr[j])
                        res_reverse = (double)resr[j];
                res_forward = 0.0;
                for (j = 0; j < iend; j++)
                    if (res_forward < (double)resf[j])
                        res_forward = (double)resf[j];
                res_max = 1.0;
            }
                ...
                res_reverse = 0.0;
                for (j = 0; j < iend; j++)
                    if (res_reverse < (double)(resr[j] - resf[j]))
                        res_reverse = (double)(resr[j] - resf[j]);
                res_forward = 0.0;
                for (j = 0; j < iend; j++)
                    if (res_forward < (double)(resf[j] - resr[j]))
                        res_forward = (double)(resf[j] - resr[j]);
                res_max = 1.0;
            }
