...
int
intcompare(const int* a, const int* b) {
    return (*a - *b);
}

int
lcompare(const Scores* a, const Scores* b) {
    if (a->orilen < b->orilen)
        return 1;
    else if (a->orilen > b->orilen)
        return -1;
    else
        return 0;
}
...
int
dcompare(const Scores* a, const Scores* b) {
    if (a->score > b->score)
        return 1;
        ...
        if (a->selfscore < b->selfscore)
            return 1;
        else if (a->selfscore > b->selfscore)
            return -1;
        else {
            if (a->orilen < b->orilen)
                return 1;
...
static void
getfastascoremtx(int** tmpaminodis) {
    FILE*        qfp;
    FILE*        dfp;
    FILE*        rfp;
    int          i, j;
    ...
    if (!dfp)
        ErrorExit("Cannot open datafile.");
    sprintf(tmpname, ">+===========+%d                      ", 0);
    ...
    for (i = 0; i < 20; i++) {
        ...
        *tmpseq = 0;
        ...
        if (!qfp)
            ErrorExit("Cannot open queryfile.");
...
static double*
callfasta(char** seq, Scores* scores, int nin, int* picks, int query, int rewritedata) {
    double*        val;
    FILE*          qfp;
    FILE*          dfp;
    FILE*          rfp;
...
void
makecompositiontable_p(short* table, int* pointt) {
    int point;
...
static int
localcommonsextet_p(short* table, int* pointt) {
    int           value = 0;
    short         tmp;
    int           point;
...
#if 1
static void
pairalign(int nseq, int* nlen, char** seq, int* mem1, int* mem2, double* weight, int* alloclen) {
    int            l, len1, len2;
    int            clus1, clus2;
...
#if 1
static int
splitseq_mq(Scores* scores, int nin, int* nlen, char** seq, char** orialn, char** name, char* inputfile, int uniform, char** tree, int* alloclen, int* order, int* whichgroup, double* weight, int* depthpt, int qinoya) {
    int           val;
    int           ii, jj;
    int*          mptr;
...
//			fprintf( stderr, "lenfac = %f l=%d,%d\n", lenfac,scores->orilen, scores[i].orilen );
            ...
            if (doalign) {
                ...
                if (scores[i].score > MAX6DIST)
                    scores[i].score = MAX6DIST;
            }
    ...
    if (nin == 1)
        fprintf(stderr, "nin=1, scores[0].score = %f\n", scores[0].score);
...
//		fprintf( stderr, "lenfac = %f l=%d,%d\n", lenfac,scores->orilen, scores[i].orilen );
            ...
            if (doalign) {
                ...
                if (pickmtx[j][i - j] > MAX6DIST)
                    pickmtx[j][i - j] = MAX6DIST;
            }
        }
        if (doalign == 'f')
            free(blastresults);
    ...
    if (npick > 2) {
        ...
        for (i = 0; i < npick; i++) {
            if (docholist[i] == 0)
                closeh[picks[i]] = yukos[0];
            if (docholist[i] == 1)
                closeh[picks[i]] = yukos[1];
        }
...
#endif
        jptr = treeorder;
        iptr = topol[nyuko - 2][0];
        while (*iptr != -1)
            *jptr++ = *iptr++;
        iptr = topol[nyuko - 2][1];
        while (*iptr != -1)
            *jptr++ = *iptr++;
        *jptr++ = -1;
        for (j = 0; j < nyuko; j++) {
...
static void
alignparaphiles(int nseq, int* nlen, double* weight, char** seq, int nmem, int* members, int* alloclen) {
    int  i, ilim;
...
main(int argc, char* argv[]) {
    ...
    if (tokyoripara == NOTSPECIFIED) {
        if (doalign)
            tokyoripara = TOKYORIPARA_A;
        else
            tokyoripara = TOKYORIPARA;
    }
    if (picksize > njob)
        tokyoripara = 0.0;
