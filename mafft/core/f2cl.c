...
void
setmark_clustal(int nlen, int nseq, char** seq, char* mark) {
    int   i, j, k, nalpha;
    char  firstletter;
    char* strong[9];
    char* weaker[11];
    ...
    for (i = 0; i < nlen; i++) {
        ...
        if (extendedalphabet) {
            firstletter = seq[0][i];
            if (amino_n[(unsigned char)firstletter] < 0)
                continue;

            for (j = 0; j < nseq; j++)
                if (seq[j][i] != firstletter)
                    break;
            if (j == nseq) {
                mark[i] = '*';
                continue;
            ...
            if (amino_n[(unsigned char)firstletter] >= nalpha || amino_n[(unsigned char)firstletter] < 0)
                continue;

            for (j = 0; j < nseq; j++)
                if (toupper(seq[j][i]) != firstletter)
                    break;
            if (j == nseq) {
                mark[i] = '*';
                continue;
            ...
            for (k = 0; k < nstrong; k++) {
                for (j = 0; j < nseq; j++) {
                    if (!strchr(strong[k], toupper(seq[j][i])))
                        break;
                }
                if (j == nseq)
                    break;
            ...
            for (k = 0; k < nweaker; k++) {
                for (j = 0; j < nseq; j++) {
                    if (!strchr(weaker[k], toupper(seq[j][i])))
                        break;
                }
                if (j == nseq)
                    break;
...
void
setmark(int nlen, int nseq, char** seq, char* mark) {
    int i, j;
    ...
    for (i = 0; i < nlen; i++) {
        ...
        for (j = 0; j < nseq; j++)
            if (amino_grp[(unsigned char)seq[0][i]] != amino_grp[(unsigned char)seq[j][i]])
                break;
        if (j == nseq) {
            mark[i] = '.';
            continue;
...
main(int argc, char* argv[]) {
    ...
    if (excludedashseq) {
        nout = 0;
        for (i = 0; i < njob; i++) {
            if (!strncmp(name[order[i]] + 1, "DASH|", 5) || strstr(name[order[i]] + 1, "_numo_e_DASH|"))
                continue;
            nout++;
        }
