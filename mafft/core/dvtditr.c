    ...
    while (--argc > 0 && (*++argv)[0] == '-') {
        while ((c = *++argv[0])) {
            switch (c) {
                ...
                case 'p':
                    argkey = *++argv;
                    if (!strcmp(argkey, "BESTFIRST"))
                        parallelizationstrategy = BESTFIRST;
                    else if (!strcmp(argkey, "BAATARI0"))
                        parallelizationstrategy = BAATARI0;
                    else if (!strcmp(argkey, "BAATARI1"))
                        parallelizationstrategy = BAATARI1;
                    else if (!strcmp(argkey, "BAATARI2"))
...
main(int argc, char* argv[]) {
    ...
    for (i = 0; i < njob; i++)
        skipthisbranch[i][0] = skipthisbranch[i][1] = 0;
...
exit( 1 );
        ...
        } else if (intop)  // v6.528 deha if( intop ) dattanode intree ga mukou datta.
        {
            ...
            ErrorExit("Incorrect treemethod.\n");
    }
#if DEBUG
    printf("utree = %d\n", utree);
#endif
    ...
    if (autosubalignment > 0.0 && subalignment == 0) {
        ...
        for (i = 0; i < njob; i++)
            insubtable[i] = 0;
        for (i = 0; i < njob; i++)
            preservegaps[i] = 0;  // tsukawanaikamo
    ...
    if (nadd > 0) {
        ...
        for (j = 0; j < njob - 1; j++) {
            ...
            if (overlapmember(topol[j][0], reftable) && !samemember(topol[j][0], reftable))
                skipthisbranch[j][0] = 1;
            if (overlapmember(topol[j][1], reftable) && !samemember(topol[j][1], reftable))
