...
void
treebase(char** name, int nlen[M], char** seq, char** aseq, char** mseq1, char** mseq2, double** mtx, int*** topol, double** len, double** eff, int alloclen) {
    int             i, j, l;
    int             clus1, clus2;
    int             s1, s2, r1, r2;
    double          pscore;
    ...
    if (checkC)
        for (i = 0; i < njob; i++)
            fprintf(stderr, "effarr for aseq-%d %f\n", i, effarr[i]);
    ...
    for (l = 0; l < njob - 1; l++) {
        ...
        if (checkC)
            for (i = 0; i < clus1; i++)
                fprintf(stderr, "STEP%d-eff for mseq1-%d %f\n", l + 1, i, effarr1[i]);
        /*
