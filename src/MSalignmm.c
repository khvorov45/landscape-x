...
static void
creategapfreqresult(double* gapfresult, double eff1, double eff2, double* gapf1, double* gapf2, char* gaptable1, char* gaptable2) {
    int j, p;
    int alen = strlen(gaptable1);
...
static void
createogresult(double* gapfresult, double eff1, double eff2, double* ori1, double* ori2, double* gf1, double* gf2, char* gaptable1, char* gaptable2) {
    int j, p;
    int alen = strlen(gaptable1);
...
static void
createfgresult(double* gapfresult, double eff1, double eff2, double* ori1, double* ori2, double* gf1, double* gf2, char* gaptable1, char* gaptable2) {
    int j, p;
    int alen = strlen(gaptable1);
...
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
    ...
    if (ist > 0)
        headgapfreq1 = gapfreq1f[-1];
    else
        headgapfreq1 = headgapfreq1_g;
    if (jst > 0)
        headgapfreq2 = gapfreq2f[-1];
    else
        headgapfreq2 = headgapfreq2_g;
...
freearrays_rec1(
    double*  w1,
    double*  w2,
    double*  initverticalw,
    double*  lastverticalw,
    double*  midw,
    double*  midm,
    double*  midn,
    int*     jumpbacki,
...
static void
freearrays_rec2(char* gaps, char** aseq1, char** aseq2) {
    free(gaps);
#if MEMSAVE
    free(aseq1);
    free(aseq2);
...
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
    ...
    if (legacygapcost == 0) {
        ...
        for (i = 0; i < lgth1 + 1; i++)
            gapfreq1f[i] = 1.0 - gapfreq1f[i];
        for (i = 0; i < lgth2 + 1; i++)
            gapfreq2f[i] = 1.0 - gapfreq2f[i];
        headgapfreq1 = 1.0 - headgapfreq1;
        headgapfreq2 = 1.0 - headgapfreq2;
    } else {
        for (i = 0; i < lgth1 + 1; i++)
...
static double
MSalignmm_tanni_variousdist(double*** matrices, int icyc, int jcyc, char** seq1, char** seq2, double*** cpmx1s, double*** cpmx2s, int ist, int ien, int jst, int jen, int alloclen, int fulllen1, int fulllen2, char** mseq1, char** mseq2, double** gapinfo, int headgp, int tailgp, double headgapfreq1_g, double headgapfreq2_g)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
    //	int k;
    register int i, j, c;
...
double
MSalignmm_variousdist(double** pairoffset, double*** matrices, double** dummy_mtx, char** seq1, char** seq2, double* eff1, double* eff2, double** eff1s, double** eff2s, int icyc, int jcyc, int alloclen, char* sgap1, char* sgap2, char* egap1, char* egap2, int* chudanpt, int chudanref, int* chudanres, int headgp, int tailgp)
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
