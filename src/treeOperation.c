...
void
checkMinusLength(int nseq, double** len) {
    int i, j;
    for (i = 0; i < nseq - 1; i++)
        for (j = 0; j < 2; j++)
            if (len[i][j] < MINLEN)
...
void
negativeMember2(int* mem, int* query, int locnseq) {
    int*  ptr;
    char* tmp;
    int   i;
    int   n;
...
int*
negativeMember(int* query, int locnseq) {
    int * bk, *value = NULL;
    char* tmp;
    int   i;
    int   n;
    ...
    if (!value)
        ErrorExit("Cannot allocate value");
...
int
IntExistsInVec(int query, int* vector) {
    while (*vector != -1)
        if (query == *vector++)
            return (1);
    return (0);
...
NodeInCub
searchParent(int top, int*** topol, int Start, int End) {
    int       i, j;
    NodeInCub value;
    for (i = Start; i < End; i++) {
        for (j = 0; j < 2; j++) {
...
void
treeCnv(Node* stopol, int locnseq, int*** topol, double** len, double** bw) {
    ...
    if (!count)
        ErrorExit("Cannot allocate count.\n");
...
*/
    ...
    if (stopol[locnseq - 2].children[0] == &stopol[locnseq - 3])
        i = 1;
    else if (stopol[locnseq - 2].children[1] == &stopol[locnseq - 3])
        i = 0;
    else
        ErrorExit("ERROR in stopol ?\n");
...
double
syntheticLength(Node* ob, Node* oppositeNode) {
    int    i, count;
    int    dir_ch[3];
...
#endif
    if (tmpvalue0)
        tmpvalue0 = 1.0 / tmpvalue0;
    else
        nanflag = 1;
    if (tmpvalue1)
        tmpvalue1 = 1.0 / tmpvalue1;
    else
        nanflag = 1;
...
calcW(Node* ob, Node* op) {
    ...
    if (count != 2)
        ErrorExit("Invalid call of calcW\n");
...
void
branchWeightToPairWeight(int locnseq, int*** topol, double** pw, double** bw) {
    int i, j, k, n0, n1;
#if 0
    ...
    for (i = 0; i < locnseq - 1; i++) {
        for (j = 0; (n0 = topol[i][0][j]) != -1; j++)
            for (k = 0; (n1 = topol[i][1][k]) != -1; k++)
                pw[MIN(n0, n1)][MAX(n0, n1)] = wFromLeaf[n0] * wFromLeaf[n1] * bw[i][0] * bw[i][1];
        for (j = 0; (n0 = topol[i][0][j]) != -1; j++)
            wFromLeaf[n0] *= bw[i][0];
...
void
distFromABranch(int nseq, double* result, Node* stopol, int*** topol, double** len, int step, int LorR) {
    Node *topNode, *btmNode;
    int   i;
    ...
    if (step == nseq - 2) {
        ...
        if (i == 3)
            ErrorExit("Incorrect call of distFromABranch.");
...
void
weightFromABranch(int nseq, double* result, Node* stopol, int*** topol, int step, int LorR) {
    Node *topNode, *btmNode;
    int   i;
    ...
    if (step == nseq - 2) {
        ...
        if (i == 3)
            ErrorExit("Incorrect call of weightFromABranch.");
        btmNode = stopol[step].children[i];
        topNode = stopol + step;
    }
...
void
assignstrweight(int nseq, double* strweight, Node* stopol, int*** topol, int step, int LorR, char* kozoari, double* seqweight) {
    Node *topNode, *btmNode;
    int   i;
