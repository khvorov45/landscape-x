...
void
keika(char* str, int current, int all) {
    if (current == 0)
...
double
maxItch(double* soukan, int size) {
    int    i;
    double value = 0.0;
    double cand;
    for (i = 0; i < size; i++)
...
void
calcNaiseki(Fukusosuu* value, Fukusosuu* x, Fukusosuu* y) {
    value->R = x->R * y->R + x->I * y->I;
    value->I = -x->R * y->I + x->I * y->R;
}

Fukusosuu*
AllocateFukusosuuVec(int l1) {
    Fukusosuu* value;
...
Fukusosuu**
AllocateFukusosuuMtx(int l1, int l2) {
    Fukusosuu** value;
    int         j;
...
Fukusosuu***
AllocateFukusosuuCub(int l1, int l2, int l3) {
    Fukusosuu*** value;
    int          i;
...
void
FreeFukusosuuVec(Fukusosuu* vec) {
    free((void*)vec);
}

void
FreeFukusosuuMtx(Fukusosuu** mtx) {
    int i;
...
int
getKouho(int* kouho, int nkouho, double* soukan, int nlen2) {
    int    i, j;
    int    nlen4 = nlen2 / 2;
    double max;
    double tmp;
...
void
zurasu2(int lag, int clus1, int clus2, char** seq1, char** seq2, char** aseq1, char** aseq2) {
    int i;
#if 0
...
void
zurasu(int lag, int clus1, int clus2, char** seq1, char** seq2, char** aseq1, char** aseq2) {
    int i;
#if DEBUG
    fprintf(stderr, "lag = %d\n", lag);
#endif
...
static int
permit(Segment* seg1, Segment* seg2) {
    return (0);
...
*/
                if (k && k < *ncut - 1 && j < *ncut - 1 && !permit(seg1[k - 1], seg1[j - 1]))
                    continue;
            ...
            klim = i - 2;
            for (k = 0; k < klim; k++) {
                if (k && k < *ncut - 1 && i < *ncut - 1 && !permit(seg2[k - 1], seg2[i - 1]))
                    continue;
