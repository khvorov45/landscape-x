...
static int
addlocalhom_r(char* al1, char* al2, LocalHom* localhompt, int off1, int off2, int skip, char korh) {
    int       pos1, pos2, start1, start2, end1, end2;
    char *    pt1, *pt2;
    double    score;
    double    sumscore;
...
void
putlocalhom3(char* al1, char* al2, LocalHom* localhompt, int off1, int off2, char korh) {
    int       pos1, pos2, start1, start2, end1, end2;
    char *    pt1, *pt2;
    double    score;
    double    sumscore;
...
void
PreRead(FILE* fp, int* locnjob, int* locnlenmax) {
    int  i, nleni;
    char b[B];
...
void
readDataforgaln(FILE* fp, char** name, int* nlen, char** seq) {
    int          i;
...
void
readData_varlen(FILE* fp, char** name, int* nlen, char** seq) {
    int          i;
    static char* tmpseq = NULL;
...
void
readData_pointer2(FILE* fp, int nseq, char** name, int* nlen, char** seq) {
    int          i;
    static char* tmpseq = NULL;
...
void
readData_pointer_casepreserve(FILE* fp, char** name, int* nlen, char** seq) {
    int          i;
    static char* tmpseq = NULL;
...
int
copydatafromgui(char** namegui, char** seqgui, char** name, int* nlen, char** seq) {
    int i;
...
void
readData_pointer(FILE* fp, char** name, int* nlen, char** seq) {
    int          i;
    static char* tmpseq = NULL;
...
void
readData(FILE* fp, char name[][B], int nlen[], char** seq) {
    int          i;
    static char* tmpseq = NULL;
...
void
cutAlignment(FILE* fp, int** regtable, char** revtable, int* outtable, char** name, char** outseq) {
    int          i, j;
    int          outlen;
    static char* tmpseq = NULL;
    ...
    for (i = 0; i < njob; i++) {
        ...
        if (outtable[i]) {
            ...
            lpos = 0;
            for (j = 0; j < 5; j++) {
                if (regtable[0][j * 2] == -1 && regtable[0][j * 2 + 1] == -1)
                    continue;
...
void
cutData(FILE* fp, int** regtable, char** revtable, int* outtable) {
    int          i, j;
    ...
    for (i = 0; i < njob; i++) {
        ...
        if (outtable[i]) {
            ...
            if (outtable[i] == 2) {
                ...
                for (j = 0; j < 5; j++) {
                    if (regtable[i][j * 2] == -1 && regtable[i][j * 2 + 1] == -1)
                        continue;
...
void
getnumlen_casepreserve(FILE* fp, int* nlenminpt) {
    int    total;
    int    nsite = 0;
    int    atgcnum;
    int    i, tmp;
...
void
getnumlen_nogap_countn(FILE* fp, int* nlenminpt, double* nfreq) {
    int    total;
    int    nsite = 0;
    int    atgcnum, nnum, nN;
    int    i, tmp;
...
void
getnumlen_nogap(FILE* fp, int* nlenminpt) {
    int    total;
    int    nsite = 0;
    int    atgcnum;
    int    i, tmp;
...
void
getnumlen_nogap_outallreg(FILE* fp, int* nlenminpt) {
    int    total;
    int    nsite = 0;
    int    atgcnum;
    int    i, tmp;
...
void
getnumlen_nogap_outallreg_web(FILE* fp, FILE* ofp, int* nlenminpt, int* isalignedpt) {
    int    total;
    int    nsite = 0;
    int    atgcnum;
...
void
WriteFloatHat2_pointer_halfmtx(FILE* hat2p, int locnjob, char** name, double** mtx) {
    int    i, j, ijsa;
    double max = 0.0;
...
void
WriteFloatHat2_pointer(FILE* hat2p, int locnjob, char** name, double** mtx) {
    int    i, j;
    double max = 0.0;
    for (i = 0; i < locnjob - 1; i++)
        for (j = 1; j < locnjob - i; j++)
...
void
WriteFloatHat2(FILE* hat2p, int locnjob, char name[M][B], double** mtx) {
    int    i, j;
    double max = 0.0;
    for (i = 0; i < locnjob - 1; i++)
        for (j = 1; j < locnjob - i; j++)
...
void
WriteHat2_int(FILE* hat2p, int locnjob, char name[M][B], int** mtx) {
    int    i, j;
    double max = 0.0;
    for (i = 0; i < locnjob - 1; i++)
...
void
WriteHat2(FILE* hat2p, int locnjob, char name[M][B], double** mtx) {
    int    i, j;
    double max = 0.0;
    for (i = 0; i < locnjob - 1; i++)
        for (j = i + 1; j < locnjob; j++)
...
int
ReadFasta_sub(FILE* fp, double* dis, int nseq, char name[M][B]) {
    int  i, count = 0;
    char b[B];
    int  junban[M];
...
ReadBlastm7(FILE* fp, double* dis, LocalHom* localhomlist) {
    ...
    while (1) {
        ...
        *pt = 0;
        ...
        nlocalhom += addlocalhom_r(qal, tal, localhomlist + junban[count], qstart, tstart, nlocalhom, 'h');
...
int
ReadFasta34m10_scoreonly_nuc(FILE* fp, double* dis, int nin) {
    int    count = 0;
    char   b[B];
    char*  pt;
    int    pos;
...
int
ReadFasta34m10_scoreonly(FILE* fp, double* dis, int nin) {
    int    count = 0;
    char   b[B];
    char*  pt;
    int    pos;
