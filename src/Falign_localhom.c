...
Falign_localhom(int** whichmtx, double*** scoringmatrices, char** seq1, char** seq2, double* eff1, double* eff2, double** eff1s, double** eff2s, int clus1, int clus2, int alloclen, int constraint, double* totalimpmatch, int* gapmap1, int* gapmap2, int* chudanpt, int chudanref, int* chudanres) {
    ...
    if (seq1 == NULL) {
        if (result1) {
            ...
            partA__align(NULL, NULL, NULL, NULL, 0, 0, 0, 0, NULL, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL);
...
#define CAND 0
    ...
    *totalimpmatch = 0.0;
    for (i = 0; i < count - 1; i++) {
        ...
        if (kobetsubunkatsu)
            commongappick_record(clus1, tmpres1, gapmap1);
        for (j = 0; j < clus2; j++) {
        ...
        switch (alg) {
            ...
            case ('A'):
                if (scoringmatrices)  // called by tditeration.c
                {
                    ...
                    totalscore += partA__align(tmpres1, tmpres2, eff1, eff2, clus1, clus2, alloclen, constraint, &impmatch, cut1[i], cut2[i], gapmap1, gapmap2, sgap1, sgap2, egap1, egap2);
