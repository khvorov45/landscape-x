...
void
imp_match_init_strictD(aln_Opts opts, Context* ctx, int clus1, int clus2, int lgth1, int lgth2, char** seq1, char** seq2, double* eff1, double* eff2, double* eff1_kozo, double* eff2_kozo, LocalHom*** localhom, char* swaplist, int* orinum1, int* orinum2, int* uselh, int* seedinlh1, int* seedinlh2, int nodeid, int nfiles) {
    ...
    if (nodeid == -1)
        fillimp(opts, impmtx, clus1, clus2, lgth1, lgth2, seq1, seq2, eff1, eff2, eff1_kozo, eff2_kozo, localhom, swaplist, orinum1, orinum2);
    else
...
#if USEGAPLENHALFORMTX
static void
FreeGaplenMtxReport(Gaplen** mtx) {
    int i;
    if (mtx == NULL)
        return;
...
#if USEGAPLENHALFORMTX
static void
FreeGaplenCubgaplenReport(Gaplen*** cub) {
    int i;
    if (cub == NULL)
        return;
...
static int
strralpha(const char* s, const char* first) {
    int v = 0;
    s--;
    while (s >= first) {
        if (*s-- != '-')
...
#if DEBUG
static void
showgaplen(Gaplen** mtx, int seqlen) {
    int i, l;
#if USEGAPLENHALFORMTX
    int id, pos;
#endif
...
calcpfac_gap_noidatend(Gaplen** gaplen1, Gaplen** gaplen2, int newgaplen, int i, int j) {
    ...
    pfac10 = 0.0;
    if (gaplen1i)
        for (k = 0; (g1 = gaplen1i + k)->idatnext != -1; k++) {
            pos1 = g1->relend;
            if (pos1 != 0) {
                pfac2 = 0.0;
                if (gaplen2j)
                    for (l = 0; (g2 = gaplen2j + l)->idatnext != -1; l++) {
                        pos2 = g2->relend;
                ...
                if (gaplen2j)
                    for (l = 0; (g2 = gaplen2j + l)->idatnext != -1; l++) {
                        pos2 = g2->relend;
                        if (pos2 == 0 && g2->len == g1->len + newgaplen)
                            pfac2 -= g2->freq;  // kokode shuryou suru gap, gaplen1 ha kangaenai.
                        if (pos2 != 0 && g2->len - (pos2 - 1) > g1->len + newgaplen)
                            pfac2 -= g2->freq;  // keizoku suru gap, gaplen1 ha kangaenai.
                    }
    ...
    if (gaplen2j)
        for (l = 0; (g2 = gaplen2j + l)->idatnext != -1; l++) {
            pos2 = g2->relend;
            if (pos2 == 0 && g2->len == newgaplen)
                pfac2 -= g2->freq;  // kokode shuryou suru gap, gaplen1 ha kangaenai.
            if (pos2 != 0 && g2->len - (pos2 - 1) > newgaplen)
                pfac2 -= g2->freq;  // keizoku suru gap, gaplen1 ha kangaenai.
        }
#if DEBUG
    ...
    pfac10 = 0.0;
    if (gaplen1i)
        for (k = 0; (gaplen1i[k].idatnext) != -1; k++) {
            pos1 = gaplen1i[k].relend;
            if (pos1 != 0) {
                pfac2 = 0.0;
                if (gaplen2j)
                    for (l = 0; (gaplen2j[l].idatnext) != -1; l++) {
                ...
                if (gaplen2j)
                    for (l = 0; (gaplen2j[l].idatnext) != -1; l++) {
                        pos2 = gaplen2j[l].relend;
                        if (pos2 == 0 && gaplen2j[l].len == gaplen1i[k].len + newgaplen)
                            pfac2 -= gaplen2j[l].freq;  // kokode shuryou suru gap, gaplen1 ha kangaenai.
                        if (pos2 != 0 && gaplen2j[l].len - (pos2 - 1) > gaplen1i[k].len + newgaplen)
                            pfac2 -= gaplen2j[l].freq;  // keizoku suru gap, gaplen1 ha kangaenai.
                    }
                //			reporterr( "pfac2 in line 1056 = %f\n", pfac2 );
                pfac += pfac2 * gaplen1i[k].freq;
    ...
    if (gaplen2j)
        for (l = 0; (gaplen2j[l].idatnext) != -1; l++) {
            pos2 = gaplen2j[l].relend;
            if (pos2 == 0 && gaplen2j[l].len == newgaplen)
                pfac2 -= gaplen2j[l].freq;  // kokode shuryou suru gap, gaplen1 ha kangaenai.
            if (pos2 != 0 && gaplen2j[l].len - (pos2 - 1) > newgaplen)
                pfac2 -= gaplen2j[l].freq;  // keizoku suru gap, gaplen1 ha kangaenai.
        }
#if DEBUG
    reporterr("pfac1 (type3) = %f\n", pfac1);
...
static double
calcpfac_gap_incomplete(Gaplen** gaplen1, Gaplen** gaplen2, int newgaplen, int i, int j, char* seq1, char* seq2, int disp)  // seq1 to seq2 ha debug you
{
    double  pfac, pfac1, pfac10, pfac2;
    int     k, l, pos1, pos2, id1, id2;
    ...
    pfac10 = 0.0;
    if (gaplen1i)
        for (k = 0; (id1 = gaplen1i[k].idatend) != -1; k++) {
            pos1 = gaplen1i[k].relend;
            gapend1 = gaplen1[i + pos1] + id1;
            if (pos1 != 0) {
                pfac2 = 0.0;
                if (gaplen2j)
                    for (l = 0; (id2 = gaplen2j[l].idatend) != -1; l++) {
                        pos2 = gaplen2j[l].relend;
                ...
                if (gaplen2j)
                    for (l = 0; (id2 = gaplen2j[l].idatend) != -1; l++) {
                        pos2 = gaplen2j[l].relend;
                        gapend2 = gaplen2[j + pos2] + id2;
                        if (pos2 == 0 && gapend2->len == gapend1->len + newgaplen)
                            pfac2 -= gapend2->freq;  // kokode shuryou suru gap, gaplen1 ha kangaenai.
                        if (pos2 != 0 && gapend2->len - (pos2 - 1) > gapend1->len + newgaplen)
                            pfac2 -= gapend2->freq;  // keizoku suru gap, gaplen1 ha kangaenai.
                    }
                //			reporterr( "pfac2 in line 1056 = %f\n", pfac2 );
                pfac += pfac2 * gapend1->freq;
                pfac10 += gapend1->freq;
    ...
    if (gaplen2j)
        for (l = 0; (id2 = gaplen2j[l].idatend) != -1; l++) {
            pos2 = gaplen2j[l].relend;
            gapend2 = gaplen2[j + pos2] + id2;
            if (pos2 == 0 && gapend2->len == newgaplen)
                pfac2 -= gapend2->freq;  // kokode shuryou suru gap, gaplen1 ha kangaenai.
            if (pos2 != 0 && gapend2->len - (pos2 - 1) > newgaplen)
                pfac2 -= gapend2->freq;  // keizoku suru gap, gaplen1 ha kangaenai.
        }
#if DEBUG
    reporterr("pfac1 (type3) = %f\n", pfac1);
...
static double
calcpfac_gapex(Gaplen** gaplen1, Gaplen** gaplen2, int i, int j, int newgaplen, char* seq1, char* seq2, int disp) {
    ...
    if (gaplen2j)
        for (k = 0; (id2 = gaplen2j[k].idatend) != -1; k++)  // ososugi!  hash ni atode henkou
        {
#if DEBUG
            int gl;
            ...
            pfac10 = 1.0;
            if (gaplen1i)
                for (l = 0; (id1 = gaplen1i[l].idatend) != -1; l++)  // ososugi!  hash ni atode henkou
                {
...
#endif
                    if (newgaplen + gapend1->len - (pos1) > gapend2->len - (pos2))
                        pfac1 -= gapend1->freq;
                    //			reporterr( "pfac1 = %f\n", pfac1 );
                }
...
static double
calcpfac(Gaplen** gaplen1, Gaplen** gaplen2, int i, int j, char* seq1, char* seq2, int disp)  // seq1 to seq2 ha debug you
{
    ...
    if (gaplen1i)
        for (k = 0; (id1 = gaplen1i[k].idatend) != -1; k++) {
            if ((pos1 = gaplen1i[k].relend) == 0)
                pfac1 += gaplen1[i + pos1][id1].freq;
        }

    if (gaplen2j)
        for (l = 0; (id2 = gaplen2j[l].idatend) != -1; l++)  // ososugi!  hash ni atode henkou
        {
            if ((pos2 = gaplen2j[l].relend) == 0)
                pfac2 += gaplen2[j + pos2][id2].freq;
        }
#if DEBUG
    ...
    if (gaplen1i)
        for (k = 0; (id1 = gaplen1i[k].idatend) != -1; k++)  // ososugi!  hash ni atode henkou
        {
            pos1 = gaplen1i[k].relend;
            gapend1 = gaplen1[i + pos1] + id1;
            if (gaplen2j)
                for (l = 0; (id2 = gaplen2j[l].idatend) != -1; l++)  // ososugi!  hash ni atode henkou
                {
                    pos2 = gaplen2j[l].relend;
                    gapend2 = gaplen2[j + pos2] + id2;
                    if (pos1 == 0 && pos2 == 0 && gapend1->len == gapend2->len)
                        pfac -= gapend1->freq * gapend2->freq;
                    else if (pos1 == 0 && pos2 != 0 && gapend2->len - (pos2 - 1) > gapend1->len)
                        pfac -= gapend1->freq * gapend2->freq;
                    else if (pos1 != 0 && pos2 == 0 && gapend1->len - (pos1 - 1) > gapend2->len)
                        pfac -= gapend1->freq * gapend2->freq;
                }
        }
...
calcpfac_gapex_noidatend(Gaplen** gaplen1, Gaplen** gaplen2, int i, int j, int newgaplen) {
    ...
    if (gaplen2j)
        for (k = 0; (g2 = gaplen2j + k)->idatnext != -1; k++) {
            ...
            pfac10 = 1.0;
            if (gaplen1i)
                for (l = 0; (g1 = gaplen1i + l)->idatnext != -1; l++) {
                    pos1 = g1->relend;
                    pfac10 -= g1->freq;
#if DEBUG
                    if (disp)
...
calcpfacnoidatend(Gaplen** gaplen1, Gaplen** gaplen2, int i, int j) {
    ...
    if (gaplen2j)
        for (l = 0; (g2 = gaplen2j + l)->idatnext != -1; l++)  // ososugi!  hash ni atode henkou
        {
            if ((pos2 = g2->relend) == 0)
                pfac2 += g2->freq;
        }
    ...
    //	if( pfac ) reporterr( "i,j=%d,%d, Cancel (eq len)? pfac = %f -> ", i, j, pfac );
    if (gaplen1i)
        for (k = 0; (g1 = gaplen1i + k)->idatnext != -1; k++)  // ososugi!  hash ni atode henkou
        {
            pos1 = g1->relend;
            if (gaplen2j)
                for (l = 0; (g2 = gaplen2j + l)->idatnext != -1; l++)  // ososugi!  hash ni atode henkou
                {
                    pos2 = gaplen2j[l].relend;
                    if (pos1 == 0 && pos2 == 0 && g1->len == g2->len)
                        pfac -= g1->freq * g2->freq;
                    else if (pos1 == 0 && pos2 != 0 && g2->len - (pos2 - 1) > g1->len)
                        pfac -= g1->freq * g2->freq;
                    else if (pos1 != 0 && pos2 == 0 && g1->len - (pos1 - 1) > g2->len)
                        pfac -= g1->freq * g2->freq;
                }
        }
    ...
    if (gaplen1i)
        for (k = 0; (gaplen1i[k].idatnext) != -1; k++) {
            if (gaplen1i[k].relend == 0)
                pfac1 += gaplen1[i][k].freq;
        }

    if (gaplen2j)
        for (l = 0; (gaplen2j[l].idatnext) != -1; l++)  // ososugi!  hash ni atode henkou
        {
            if (gaplen2j[l].relend == 0)
                pfac2 += gaplen2[j][l].freq;
        }
#if DEBUG
    reporterr("\n\nInitial pfac1,pfac2 (%c%d,%c%d) = %f, %f\n", seq1[i], i, seq2[j], j, pfac1, pfac2);
    ...
    //	if( pfac ) reporterr( "i,j=%d,%d, Cancel (eq len)? pfac = %f -> ", i, j, pfac );
    if (gaplen1i)
        for (k = 0; (gaplen1i[k].idatnext) != -1; k++)  // ososugi!  hash ni atode henkou
        {
            pos1 = gaplen1i[k].relend;
            if (gaplen2j)
                for (l = 0; (gaplen2j[l].idatnext) != -1; l++)  // ososugi!  hash ni atode henkou
                {
                    pos2 = gaplen2j[l].relend;
                    if (pos1 == 0 && pos2 == 0 && gaplen1i[k].len == gaplen2j[l].len)
                        pfac -= gaplen1i[k].freq * gaplen2j[l].freq;
                    else if (pos1 == 0 && pos2 != 0 && gaplen2j[l].len - (pos2 - 1) > gaplen1i[k].len)
                        pfac -= gaplen1i[k].freq * gaplen2j[l].freq;
                    else if (pos1 != 0 && pos2 == 0 && gaplen1i[k].len - (pos1 - 1) > gaplen2j[l].len)
                        pfac -= gaplen1i[k].freq * gaplen2j[l].freq;
                }
        }
#endif
...
static void
extendgaplencompactx(Gaplen** cpy, Gaplen** orig, int start) {
    Gaplen *opt, *cpt;
    int     l, id;
#if DEBUG
    Gaplen cpybk;
...
#if USEGAPLENHALFORMTX
static void
extendgaplenpartly(Gaplen** cpy, Gaplen** orig, int start, int end) {
...
#if USEGAPLENHALFORMTX
static void
duplicategaplenpartly(Gaplen** cpy, Gaplen** orig, int start, int end) {
    int     i, l, gl, extrascope;
    Gaplen* pt;
...
static void
gaplenextendnoidatend(Gaplen** cpy, int gapstartpos, int insertionlen) {
...
//	reporterr( "inserting %d gaps at position %d\n", insertionlen, gapstartpos );
    ...
    for (l = 0; cpy[gapstartpos] && (id = cpy[gapstartpos][l].idatnext) != -1; l++) {
        ...
        if (gapstartpos != 0 && idn != -1 && cpy[gapstartpos - 1])
            cpy[gapstartpos - 1][idn].len = len;
    }
#endif
}
...
static void
gaplenextend(Gaplen** cpy, int gapstartpos, int insertionlen) {
    int l, id, idn, pos, len;
...
static void
copygaplencompactx(Gaplen** cpy, Gaplen** orig, int gapstartpos, int insertionlen, int posincopy, int posinori) {
    Gaplen *pt, *cpt;
...
#if USEGAPLENHALF
static void
copygaplenrestricted_zurasu(Gaplen** cpy, Gaplen** orig, int seqlen, int gapstartpos, int insertionlen, int startincopy, int endincopy, int startinori, int endinori) {
...
#if USEGAPLENHALFORMTX
static void
copygaplenrestricted(Gaplen** cpy, Gaplen** orig, int seqlen, int gapstartpos, int insertionlen, int scopestart, int scopeend) {
...
#if 1
static void
freegaplenpartly(Gaplen** mtx, int startpos, int endpos) {
    int      i;
    Gaplen** pt;
    if (startpos < 0)
        startpos = 0;
...
double
D__align(aln_Opts opts, Context* ctx, double** n_dynamicmtx, char** seq1, char** seq2, double* eff1, double* eff2, int icyc, int jcyc, int alloclen, int constraint, double* impmatch, int headgp, int tailgp) {
    //	int k;
    register int i, j;
...
//	double fpenalty = (double)penalty;
    ...
    if (seq1 == NULL) {
        if (orlgth1) {
            ...
            free(gaplens);
            if (gaplen1ibestkamo)
                FreeGaplenCub(gaplen1ibestkamo);
            gaplen1ibestkamo = NULL;
            if (gaplen2ibestkamo)
                FreeGaplenCub(gaplen2ibestkamo);
            gaplen2ibestkamo = NULL;
            if (gaplen1icurr)
                FreeGaplenCub(gaplen1icurr);
            gaplen1icurr = NULL;
            if (gaplen2icurr)
                FreeGaplenCub(gaplen2icurr);
            gaplen2icurr = NULL;

            if (gaplen1jprev)
                FreeGaplenCub(gaplen1jprev);
            gaplen1jprev = NULL;
            if (gaplen2jprev)
                FreeGaplenCub(gaplen2jprev);
            gaplen2jprev = NULL;
            if (gaplen1jcurr)
                FreeGaplenCub(gaplen1jcurr);
            gaplen1jcurr = NULL;
            if (gaplen2jcurr)
                FreeGaplenCub(gaplen2jcurr);
            gaplen2jcurr = NULL;
            if (gaplen1jbestkamo)
                FreeGaplenCub(gaplen1jbestkamo);
            gaplen1jbestkamo = NULL;
            if (gaplen2jbestkamo)
                FreeGaplenCub(gaplen2jbestkamo);
            gaplen2jbestkamo = NULL;
            if (gaplen1jbest)
                FreeGaplenCub(gaplen1jbest);
            gaplen1jbest = NULL;
            if (gaplen2jbest)
    ...
    for (i = 0; i < lgth1 + 1; i++)
        gaplen1mtx[i] = (Gaplen***)calloc(lgth2 + 2, sizeof(Gaplen**));
    for (i = 0; i < lgth1 + 1; i++) {
        for (j = 0; j < lgth2 + 1; j++) {
            gaplen1mtx[i][j] = (Gaplen**)calloc(lgth1 + 2, sizeof(Gaplen*));
            for (k = 0; k < lgth1 + 1; k++)
                gaplen1mtx[i][j][k] = NULL;
    ...
    for (i = 0; i < lgth1 + 1; i++)
        gaplen2mtx[i] = (Gaplen***)calloc(lgth2 + 2, sizeof(Gaplen**));
    for (i = 0; i < lgth1 + 1; i++) {
        for (j = 0; j < lgth2 + 1; j++) {
            gaplen2mtx[i][j] = (Gaplen**)calloc(lgth2 + 2, sizeof(Gaplen*));
            for (k = 0; k < lgth2 + 1; k++)
                gaplen2mtx[i][j][k] = NULL;
    ...
    for (i = 0; i < lgth1 + 1; i++)
        gaplen1half[i] = (Gaplen***)calloc(lgth2 + 2, sizeof(Gaplen**));
    for (i = 0; i < lgth1 + 1; i++) {
        for (j = 0; j < lgth2 + 1; j++) {
            gaplen1half[i][j] = (Gaplen**)calloc(lgth1 + 2 - i, sizeof(Gaplen*));
            for (k = 0; k < lgth1 + 1 - i; k++)
                gaplen1half[i][j][k] = NULL;
    ...
    for (i = 0; i < lgth1 + 1; i++)
        gaplen2half[i] = (Gaplen***)calloc(lgth2 + 2, sizeof(Gaplen**));
    for (i = 0; i < lgth1 + 1; i++) {
        for (j = 0; j < lgth2 + 1; j++) {
            gaplen2half[i][j] = (Gaplen**)calloc(lgth2 + 2 - j, sizeof(Gaplen*));
            for (k = 0; k < lgth2 + 1 - j; k++)
                gaplen2half[i][j][k] = NULL;
...
#endif
                copygaplencompactx(gaplen1jbest[j - 1], gaplen1jbestkamo[j - 1], -1, 0, i, i);  // *mpjpt, i
                copygaplencompactx(gaplen2jbest[j - 1], gaplen2jbestkamo[j - 1], -1, 0, 1, 1);  // half!
            }
...
//				freegaplenpartly( gaplen2jbestkamo[j-1], j-3, j-2 );
                ...
                copygaplencompactx(gaplen1jbestkamo[j - 1], gaplen1jprev[j - 1], -1, 0, i, i);  // *mpjpt, i
                copygaplencompactx(gaplen2jbestkamo[j - 1], gaplen2jprev[j - 1], -1, 0, 1, 1);  // ryouhou half!
            }
...
#endif
                copygaplencompactx(gaplen1jcurr[j], gaplen1jprev[j + *ijppt], i, -*ijppt - 1, i, i);  // scope: i+*ijppt+1, i ?
                copygaplencompactx(gaplen2jcurr[j], gaplen2jprev[j + *ijppt], -1, 0, 0, -*ijppt);  // half! ryouho zureteru
            } else if (*ijppt > 0) {
#if DEBUG
...
#endif
                copygaplencompactx(gaplen1jcurr[j], gaplen1jbest[j - 1], -1, 0, i, i);
                copygaplencompactx(gaplen2jcurr[j], gaplen2jbest[j - 1], 0, *ijppt - 1, 0, 1);  //ryouhou half
...
#endif
                copygaplencompactx(gaplen1jcurr[j], gaplen1jprev[j - 1], -1, 0, i, i);
                copygaplencompactx(gaplen2jcurr[j], gaplen2jprev[j - 1], -1, 0, 0, 1);  // half
            }
...
double
D__align_variousdist(aln_Opts opts, Context* ctx, int** which, double*** matrices, char** seq1, char** seq2, double* eff1, double* eff2, double** eff1s, double** eff2s, int icyc, int jcyc, int alloclen, int constraint, double* impmatch, int headgp, int tailgp) {
    //	int k;
    register int i, j, c;
    int          lasti, lastj; /* outgap == 0 -> lgth1, outgap == 1 -> lgth1+1 */
    int          lgth1, lgth2;
...
//		reporterr( "err1? i=%d/%d\n", i, lgth1 );
        ...
        if (chudanpt && *chudanpt != chudanref) {
            cleargaplens(gaplens);
            if (masklist1)
                FreeIntMtx(masklist1);
            masklist1 = NULL;
            if (masklist2)
                FreeIntMtx(masklist2);
            masklist2 = NULL;
            if (nmask)
                free(nmask);
            nmask = NULL;
