...
static void
strncpy0(char* s1, char* s2, int n) {
    while (n--)
        *s1++ = *s2++;
    *s1 = 0;
}
...
profilealignment2(int n0, int n2, char** aln0, char** aln2, int alloclen, char alg)  // n1 ha allgap
{
    ...
    newgapstr = "-";
    if (alg == 'M')
        MSalignmm(n_dis_consweight_multi, aln0, aln2, effarr0, effarr2, n0, n2, alloclen, NULL, NULL, NULL, NULL, 1, 1, NULL, NULL, NULL, 0.0, 0.0);  //outgap=1, 2014/Dec/1
    else
        A__align(n_dis_consweight_multi, penalty, penalty_ex, aln0, aln2, effarr0, effarr2, n0, n2, alloclen, 0, &dumdb, NULL, NULL, NULL, NULL, 1, 1, -1, -1, NULL, NULL, NULL, 0.0, 0.0);  //outgap=1, 2014/Dec/1
...
static void
profilealignment(int n0, int n1, int n2, char** aln0, char** aln1, char** aln2, int alloclen, char alg)  // n1 ha allgap
{
    int     i, j, newlen;
    ...
    for (i = 0; i < n0; i++)
        if (!allgap0[i])
            ...
            effarr0[i] = 0.0;  // malloc / alloca no baai
    for (i = 0; i < n2; i++)
        if (!allgap2[i])
            effarr2[i] = 1.0 / (double)(alcount2);
        else
            effarr2[i] = 0.0;  // malloc / alloca no baai
#else
    for (i = 0; i < n0; i++)
        if (!allgap0[i])
            effarr0[i] = 1.0 / (double)(alcount0);  // calloc no baai
    for (i = 0; i < n2; i++)
        if (!allgap2[i])
            effarr2[i] = 1.0 / (double)(alcount2);  // calloc no baai
    eff = 1.0 / (double)n0;
    for (i = 0; i < n0; i++)
        effarr0[i] = eff;
    eff = 1.0 / (double)n2;
...
static int
countnogaplen(int* gaplen, int* term) {
    int v = 0;
    while (gaplen < term) {
        if (*gaplen++ == 0)
            v++;
...
void
insertnewgaps(int njob, int* alreadyaligned, char** seq, int* ex1, int* ex2, int* gaplen, int* gapmap, int alloclen, char alg, char gapchar) {
    int*   mar;
    char*  gaps;
    char*  cptr;
    ...
    for (j = 0; j < len0; j++) {
        ...
        if (gaplen[j]) {
            ...
            *cptr = 0;
            ...
            for (i = 0; i < ngroup0; i++)
                strncpy0(mseq0[i] + mlen0, gaps, gapshift);
            for (i = 0; i < ngroup1; i++)
                strncpy0(mseq1[i] + mlen1, seq[list1[i]] + posin12, gapshift);
            for (i = 0; i < ngroup2; i++)
                strncpy0(mseq2[i] + mlen2, seq[list2[i]] + posin12, gapshift);
            posin12 += gapshift;
            mlen0 += gapshift;
            mlen1 += gapshift;
            mlen2 += gapshift;
            ...
            for (i = 0; i < ngroup0; i++)
                strncpy0(mseq0[i] + mlen0, seq[list0[i]] + j, gapshift);
            for (i = 0; i < ngroup1; i++)
                strncpy0(mseq1[i] + mlen1, seq[list1[i]] + posin12, gapshift);
            for (i = 0; i < ngroup2; i++)
                strncpy0(mseq2[i] + mlen2, seq[list2[i]] + posin12, gapshift);
            ...
            for (i = 0; i < ngroup0; i++)
                strcpy(aseq[list0[i]] + newpos, mseq0[i]);
            for (i = 0; i < ngroup1; i++)
...
//		fprintf( stderr, "### aseq[l2] = %s\n", aseq[list2[0]] );
    ...
    for (i = 0; i < ngroup1; i++)
        strcpy(seq[list1[i]], aseq[list1[i]]);
    for (i = 0; i < ngroup2; i++)
        strcpy(seq[list2[i]], aseq[list2[i]]);
...
void
insertnewgaps_bothorders(int njob, int* alreadyaligned, char** seq, int* ex1, int* ex2, int* gaplen, int* gapmap, int gapmaplen, int alloclen, char alg, char gapchar) {
    //	int disp = 0;
    int*   mar;
    char*  gaps;
    char*  cptr;
    ...
    for (j = 0; j < len0; j++) {
        ...
        if (gaplen[j] || gapmap[posin12]) {
            ...
            {
                for (i = 0; i < ngroup0; i++)
                    strncpy0(mseq0[i] + mlen0, seq[list0[i]] + j, gapshiftb);  // tokuni koko!
                for (i = 0; i < ngroup1; i++)
                    strncpy0(mseq1[i] + mlen1, seq[list1[i]] + posin12, gapshiftb);
                for (i = 0; i < ngroup2; i++)
                    strncpy0(mseq2[i] + mlen2, seq[list2[i]] + posin12, gapshiftb);
                mlen0 += gapshiftb;
                mlen1 += gapshiftb;
                mlen2 += gapshiftb;
#if 0
            ...
            while (1) {
                ...
                if (gapshiftn) {
                    ...
                    *cptr = 0;
                    ...
                    for (i = 0; i < ngroup0; i++)
                        strncpy0(mseq0[i] + mlen0, gaps, gapshiftn);
                    for (i = 0; i < ngroup1; i++)
                        strncpy0(mseq1[i] + mlen1, seq[list1[i]] + posin12, gapshiftn);
                    for (i = 0; i < ngroup2; i++)
                        strncpy0(mseq2[i] + mlen2, seq[list2[i]] + posin12, gapshiftn);
                    posin12 += gapshiftn;
                    mlen0 += gapshiftn;
                    mlen1 += gapshiftn;
                    mlen2 += gapshiftn;
                ...
                if (gapshifta) {
                    for (i = 0; i < ngroup0; i++)
                        strncpy0(mseq0[i] + mlen0, seq[list0[i]] + j, gapshifta);
                    for (i = 0; i < ngroup1; i++)
                        strncpy0(mseq1[i] + mlen1, seq[list1[i]] + posin12, gapshifta);
                    for (i = 0; i < ngroup2; i++)
                        strncpy0(mseq2[i] + mlen2, seq[list2[i]] + posin12, gapshifta);
                    mlen0 += gapshifta;
                    mlen1 += gapshifta;
                    mlen2 += gapshifta;
                    posin12 += gapshifta;
