...
static void
makescoringmatrices(double*** matrices, double** originalmtx) {
    int    c;
    double rep;
...
static void
classifypairs(int n1, double** eff1s, double* eff1, int n2, double** eff2s, double* eff2, double** smalldistmtx, int** matnum, int maxdistclass) {
    int i, j, c;
    for (c = 0; c < maxdistclass; c++) {
        for (i = 0; i < n1; i++)
            eff1s[c][i] = 0.0;
...
static void
Writeoption2(FILE* fp, int cycle, double cut) {
...
freelocalarrays(
    ...
    if (smalldistmtx)
        FreeDoubleMtx(smalldistmtx);
    if (scoringmatrices)
        FreeDoubleCub(scoringmatrices);
    if (eff1s)
        FreeDoubleMtx(eff1s);
    if (eff2s)
        FreeDoubleMtx(eff2s);
...
athread(void* arg) {
    ...
    if (rnakozo)
        rnapairboth = (RNApair*)calloc(alloclen, sizeof(RNApair));
            ...
            if (*collectingpt == 1) {
                ...
                *jobposintpt = 0;
                ...
                for (i = 0; i < nwa; i++)
                    tscorelist[i] = 0.0;
                for (i = 0; i < nbranch; i++)
                    generationofinput[i] = -1;
                if (parallelizationstrategy != BESTFIRST && randomseed != 0)
                    shuffle(branchtable, nbranch);
...
*/
            ...
            *collectingpt = 1;  // chofuku
            ...
            if (maxgain > 0.0) {
                ...
                if (parallelizationstrategy == BESTFIRST) {
                    for (i = 0; i < locnjob; i++)
                        strcpy(mastercopy[i], candidates[bestthread][i]);
                    if (scoreout) {
                        ...
                        if (weight || constraint)
                            fprintf(stderr, " (differs from the objective score)");
                        fprintf(stderr, "\n");
                    }
                }
#if 1
        ...
        *collectingpt = -1;
        ...
        while (1) {
            ...
            OneClusterAndTheOther_fast(locnjob, memlist[0], memlist[1], &s1, &s2, pairbuf, topol, l, k, smalldistmtx, distarr);
        ...
        yarinaoshi:
            ...
            pthread_mutex_lock(targ->mutex);
            for (i = 0; i < locnjob; i++)
                strcpy(localcopy[i], mastercopy[i]);
            ...
            if (nkozo) {
                ...
                for (i = 0; i < clus1; i++)
                    effarr1_kozo[i] *= 1.0;  // 0.5 ga sairyo ?
                //				tmptmptmp = 0.0;
                ...
                for (i = 0; i < clus2; i++)
                    effarr2_kozo[i] *= 1.0;  // 0.5 ga sairyo ?
                ...
                if (constraint) {
                    ...
                    if (use_fft) {
                        if (alg == 'Q') {
                            ...
                            if (rnakozo)
                                imp_rnaD(clus1, clus2, mseq1, mseq2, effarr1, effarr2, grouprna1, grouprna2, gapmap2, NULL);
                            ...
                            part_imp_match_init_strict(clus1, clus2, length, length, mseq1, mseq2, effarr1, effarr2, effarr1_kozo, effarr2_kozo, localhomshrink, swaplist, 1, memlist[0], memlist[1]);
                            if (rnakozo)
                                part_imp_rna(clus1, clus2, mseq1, mseq2, effarr1, effarr2, grouprna1, grouprna2, gapmap1, gapmap2, NULL);
                            for (i = length - 1; i >= 0; i--) {
                ...
                if (gain > 0) {
                    if (parallelizationstrategy == BESTFIRST) {
                        if (gain > gainlist[thread_no]) {
                            ...
                            for (i = 0; i < locnjob; i++)
                                strcpy(candidates[thread_no][i], localcopy[i]);
                        ...
                        pthread_mutex_lock(targ->mutex);
                        for (i = 0; i < locnjob; i++)
                            strcpy(mastercopy[i], localcopy[i]);
                        *subgenerationpt += 1;
...
int
TreeDependentIteration(int locnjob, char** name, int nlen[M], char** aseq, char** bseq, int*** topol, double** len, double** distmtx, int** skipthisbranch, int alloclen, LocalHom** localhomtable, RNApair*** singlerna, int nkozo, char* kozoarivec, int ntarget, int* targetmap, int* targetmapr) {
...
//	double naivescore1;
    ...
    if (1 || effarr == NULL) /* locnjob == njob ni kagiru */
    {
        ...
        gapmap2 = AllocateIntVec(alloclen);
        if (score_check == 2)
            imanoten = AllocateDoubleMtx(njob, njob);
        if (specificityconsideration != 0) {
            smalldistmtx = AllocateDoubleMtx(locnjob, locnjob);  // ookii?
            scoringmatrices = AllocateDoubleCub(maxdistclass, nalphabets, nalphabets);
            makescoringmatrices(scoringmatrices, n_dis_consweight_multi);
...
#if DEBUG + RECORD
    if (!effmtx)
        effmtx = AllocateDoubleMtx(locnjob, locnjob);
    for (i = 0; i < locnjob; i++)
        for (j = 0; j < locnjob; j++)
            effmtx[i][j] = 1.0;
#endif
                ...
                if (devide) {
                    ...
                    for (i = 0; i < locnjob - 1; i++)
                        for (j = i + 1; j < locnjob; j++) {
                            buff1[0] = buff2[0] = 0;
                            strcat(buff1, res_g[i]);
                            strcat(buff2, res_g[j]);
                            strcat(buff1, bseq[i]);
                            strcat(buff2, bseq[j]);
                            strcat(buff1, seq_g[i]);
                            strcat(buff2, seq_g[j]);
            ...
            for (jobpos = 0; jobpos < nbranch; jobpos++) {
                {
                    ...
                    if (!weight) {
                        ...
                        nodeFromABranch(locnjob, branchnode, node, topol, l, k);
...
//						fprintf( stderr, "score_check = %d\n" );
	...
	//							fprintf( stderr, "#### i=%d, initial impmatch = %f seq1 = %c, seq2 = %c\n", i, oimpmatch, mseq1[0][i], mseq2[0][i] );
                    ...
                    if (identity) {
                        tscore = mscore;
                        if (!devide)
                            fprintf(trap_g, "tscore =  %f   identical.\n", tscore);
    ...
    if (node)
        FreeIntMtx(node);
    if (branchWeight)
        FreeDoubleMtx(branchWeight);
    if (history)
        FreeFloatCub(history);
