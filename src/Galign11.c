...
#else
static void
match_calc_mtx_codon(double** codonmtx, double** mtx, double* match, char** s1, char** s2, int** codonseq1, int** codonseq2, int i1, int lgth2)
// gstart, gend wo tsukatte coding ka kakuninn suru beki
{
    char*   seq2 = s2[0];
    double* doubleptr = mtx[(unsigned char)s1[0][i1]];
    ...
    while (lgth2--) {
        ...
        //		reporterr( "%c%c%c(%d)-%c%c%c(%d)\n", s1[0][i1], s1[0][i1+1], s1[0][i1+2], codonid1, *seq2, *(seq2+1), *(seq2+2), codonid2 );
        codonmatch = 0.0;
        if (codonid1 > -1 && codonid2 > -1)
            codonmatch = codonmtx[codonid1][codonid2] * 600.0;  // codonid1>-1 ha iranai.
        *match++ = doubleptr[(unsigned char)*seq2++] + codonmatch;
    }
}
#endif
...
G__align11psg(Context* ctx, double** codonmtx, double** n_dynamicmtx, char** seq1, char** seq2, int alloclen, int headgp, int tailgp, double* gstart, double* gend) {
    ...
    if (seq1 == NULL) {
        if (orlgth1 > 0 && orlgth2 > 0) {
            ...
            if (codonseq1)
                FreeIntMtx(codonseq1);
            codonseq1 = NULL;
            if (codonseq2)
                FreeIntMtx(codonseq2);
            codonseq2 = NULL;
        }
        orlgth1 = 0;
        orlgth2 = 0;
